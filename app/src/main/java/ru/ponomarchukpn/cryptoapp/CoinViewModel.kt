...
import ru.ponomarchukpn.cryptoapp.database.AppDatabase
import ru.ponomarchukpn.cryptoapp.pojo.CoinPriceInfo
import ru.ponomarchukpn.cryptoapp.pojo.CoinPriceInfoRawData
...
class CoinViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun loadData() {
        val disposable = ApiFactory.apiService.getTopCoinsInfo(limit = 50)
            .map { single -> single.data?.map { it.coinInf?.name }?.joinToString(",").toString() }
